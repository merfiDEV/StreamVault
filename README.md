...
- общий JavaScript для всех страниц вынесен в `ui/js/shared.js`
- уведомления и ошибки теперь идут через единый toast-helper
- локализация стала строже: текст в интерфейсе берётся только из переводов
- в выборе пути сохранения появился системный диалог выбора папки
- сборка `exe` выполняется через `build.py` и кладётся в `dist/StreamVault.exe`

---

## 📝 Примечания

- если после обновления интерфейс выглядит странно, пересоберите `exe` и запустите свежий файл из `dist`
- для desktop-версии нужен Windows и `pywebview`
- язык интерфейса можно менять прямо в настройках приложения
